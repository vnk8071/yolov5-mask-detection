...
def save_one_txt(normed_pred, save_conf, shape, file):
    ...
    for *xyxy, conf, cls in normed_pred.tolist():
        xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) /
                gn).view(-1).tolist()  # normalized x y w h
...
def process_batch(detections, labels, iou_thresholds):
    ...
    x = torch.where((iou >= iou_thresholds[0]) & (
...
def run(data,
    ...
    iou_thresholds = torch.linspace(0.5, 0.95, 10).to(
    ...
    if not is_loaded_model:
        ...
        task = task if task in ('train', 'val', 'test', 'test_new') else 'val'
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        lb = [targets[targets[:, 0] == i, 1:]
              for i in range(batch_size)] if save_hybrid else []
        t3 = time_sync()

        # Note depth 8 -> 6
        out = non_max_suppression(
            out, conf_threshold, iou_threshold, labels=lb, multi_label=True)
        dt[2] += time_sync() - t3
        ...
        for si, pred in enumerate(out):
            ...
            scale_coords(img[si].shape[1:], normed_pred[:, :4],
                         shape, shapes[si][1])  # native-space pred

            # Evaluate
            if nl:
                target_boxes = xywh2xyxy(labels[:, 1:5])  # target boxes
                scale_coords(img[si].shape[1:], target_boxes,
                             shape, shapes[si][1])  # native-space labels
                labels_per_img = torch.cat(
                    (labels[:, 0:1], target_boxes), 1)  # native-space labels
                correct = process_batch(
                    normed_pred, labels_per_img, iou_thresholds)
                if plots:
                    confusion_matrix.process_batch(normed_pred, labels_per_img)
            else:
                correct = torch.zeros(
                    pred.shape[0], num_thresholds, dtype=torch.bool)

            # correct, confidence, pred_label, target_label
            stats.append(
                (correct.cpu(), pred[:, 4].cpu(), pred[:, 5].cpu(), target_class))

            # Save/log
            if save_txt:
                save_one_txt(normed_pred, save_conf, shape,
                             file=save_dir / 'labels' / (path.stem + '.txt'))
            callbacks.run('on_val_image_end', pred,
                          normed_pred, path, names, img[si])
    ...
    if len(stats) and stats[0].any():
        ...
        mp, mr, map50, map = p.mean(), r.mean(
        ), ap50.mean(), ap.mean()
        # number of targets per class
        nt = np.bincount(stats[3].astype(np.int64), minlength=num_class)
    else:
        nt = torch.zeros(1)
    ...
    if (verbose or (num_class < 50 and not is_loaded_model)) and num_class > 1 and len(stats):
        for i, c in enumerate(ap_class):
            print(print_format %
                  (names[c], num_per_class[i], nt[c], p[i], r[i], ap50[i], ap[i]))
...
def parser():
    ...
    args.add_argument('--data', type=str,
                      default='config/data_cfg.yaml', help='dataset.yaml path')
    args.add_argument('--weights', type=str,
                      help='specify your weight path', required=True)
...
def main(args):
    ...
    if args.task in ('train', 'val', 'test', 'test_new'):  # run normally
        run(**vars(args))
