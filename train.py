...
def train(hyp,
    ...
    # Scheduler
    if args.linear_lr:
        def lr_lambda(y): return (1 - y / (epochs - 1)) * \
    ...
    hyp['obj'] *= (img_size / 640) ** 2 * 3. / \
        nl  # scale to image size and layers
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if args.image_weights:
            ...
            image_weight = labels_to_image_weights(
                dataset.labels, nc=num_class, class_weights=class_weight)
            dataset.indices = random.choices(
                range(dataset.n), weights=image_weight, k=dataset.n)  # rand weighted idx
        ...
        for i, (img_batch, targets, paths, _) in plot_bar:
            ...
            if num_inters <= num_warmup_inters:
                ...
                accumulate = max(1, np.interp(num_inters, xi, [
                ...
                for j, x in enumerate(optimizer.param_groups):
                    ...
                    if 'momentum' in x:
                        x['momentum'] = np.interp(
                            num_inters, xi, [hyp['warmup_momentum'], hyp['momentum']])

            # Multi-scale
            if args.multi_scale:
                size = random.randrange(
                    img_size * 0.5, img_size * 1.5 + grid_size) // grid_size * grid_size
                scale_factor = size / max(img_batch.shape[2:])
                if scale_factor != 1:
                    new_shape = [math.ceil(
                        x * scale_factor / grid_size) * grid_size for x in img_batch.shape[2:]]
                    img_batch = F.interpolate(
                        img_batch, size=new_shape, mode='bilinear', align_corners=False)
            ...
            callbacks.run('on_train_batch_end', num_inters, model,
                          img_batch, targets, paths, plots, args.sync_bn)
...
def parser(known=False):
    ...
    args.add_argument('--name', type=str,
                      help='define your version experience', required=True)
...
def main(args, callbacks=Callbacks()):
    ...
    args.save_dir = str(increment_path(
